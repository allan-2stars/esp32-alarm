...
enum CornerType {T_R_CORNER, T_L_CORNER, B_L_CORNER, B_R_CORNER};
  ...
  public:
    static void Draw(int16_t centerX, int16_t centerY, EyeConfig *config) {
      ...
      if(config->Radius_Top > 0) {
        EyeDrawer::FillEllipseCorner(T_L_CORNER, TLc_x, TLc_y, config->Radius_Top, config->Radius_Top, 1);
        EyeDrawer::FillEllipseCorner(T_R_CORNER, TRc_x, TRc_y, config->Radius_Top, config->Radius_Top, 1);
      }
      if(config->Radius_Bottom > 0) {
        EyeDrawer::FillEllipseCorner(B_L_CORNER, BLc_x, BLc_y, config->Radius_Bottom, config->Radius_Bottom, 1);
        EyeDrawer::FillEllipseCorner(B_R_CORNER, BRc_x, BRc_y, config->Radius_Bottom, config->Radius_Bottom, 1);
      }
    }
